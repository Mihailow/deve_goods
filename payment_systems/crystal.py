...
SECRET_KEY = '24511edbfb3dfd3ca2d87cc3c95bc0067353b297'
AUTH_LOGIN = 'testovajakassa1'


class CrystallPayment:
    ...
    async def create_payment(cls, user_id: int, amount: float, comment: str):
        ...
        create_invoice_url = 'https://api.crystalpay.io/v2/invoice/create/'
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(create_invoice_url, json=json) as resp:
    ...
    async def is_payment_paid(cls, payment_id):
        check_payment_url = 'https://api.crystalpay.io/v2/invoice/info/'
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(check_payment_url, json=json) as resp:
                resp = await resp.json()
